...
// CONFIGURATION
// ============================================================================
// Config is loaded from config.js (see config.sample.js for setup instructions)
...
let gameState = {
    ...
    currentScreen: 'setup-screen'  // which screen we're on
};

// ============================================================================
// LOCAL STORAGE - Persist game state
// ============================================================================
...
function saveGameState() {
    ...
    try {
        localStorage.setItem('charades-game-state', JSON.stringify(stateToSave));
    } catch (e) {
...
function loadGameState() {
    try {
        const saved = localStorage.getItem('charades-game-state');
        if (saved) {
            const parsed = JSON.parse(saved);

            // Only restore if there's an active game
            if (parsed.isGameActive) {
                gameState.timerLength = parsed.timerLength || 60;
                gameState.totalRounds = parsed.totalRounds || 3;
                gameState.theme = parsed.theme || '';
                gameState.players = parsed.players || [];
                gameState.currentRound = parsed.currentRound || 1;
                ...
                return true; // Game state restored
            }
        }
    } catch (e) {
...
function clearGameState() {
    try {
        localStorage.removeItem('charades-game-state');
    } catch (e) {
...
async function getTopicFromAI() {
    ...
    if (!CONFIG.N8N_WEBHOOK_URL || CONFIG.N8N_WEBHOOK_URL === 'YOUR_N8N_WEBHOOK_URL_HERE') {
        console.log('n8n webhook not configured - using fallback topics');
...
function nextTurn() {
    ...
    // Save before showing next player
    saveGameState();
...
function showGameOver() {
    ...
    clearGameState(); // Clear saved game when game ends
...
// ============================================================================
// INITIALIZATION - Load saved game on page load
// ============================================================================

document.addEventListener('DOMContentLoaded', () => {
    const hasRestoredGame = loadGameState();

    if (hasRestoredGame && gameState.isGameActive) {
        // Show a notification that game was restored
        ...
        `;
        notification.textContent = 'Game restored! Continue playing.';
        ...
        // Restore to the correct screen
        showScreen(gameState.currentScreen);

        // If on game screen, restore UI
        if (gameState.currentScreen === 'game-screen') {
            displayCurrentPlayer();
            updateRoundIndicator();
        } else if (gameState.currentScreen === 'leaderboard-screen') {
            showLeaderboard();
        }
    } else {
